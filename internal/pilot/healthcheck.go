...
func RunHealthCheck(ctx context.Context, p *project.Project, cfg *config.Config, model string, timeout time.Duration) (*HealthCheckResult, error) {
	skillBody, err := loadProjectSkill("pilot-health-check")
	if err != nil {
...
// It is auto-generated by clawflow from project.yaml on every Pilot
...
func buildHealthCheckPrompt(projectName, contextMD, testingMD, deploymentMD string, repos []repoInput, skillBody string) string {
	...
	fmt.Fprintf(&b, "## Project deployment.md\n\n")
	if strings.TrimSpace(deploymentMD) == "" {
		b.WriteString("(empty / not yet generated)\n\n")
	} else {
		b.WriteString(deploymentMD)
		ensureTrailingBlank(&b)
	}
