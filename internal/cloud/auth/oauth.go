...
// handleIndex renders a minimal landing page at GET /. Used as a friendlier
// fallback while PR 1 has no Web UI mounted on the cloud server. Signed-in
// users see a confirmation panel + sign-out hint; anonymous users see a
// sign-in link. PR 2 replaces this with the real React app.
func (h *Handler) handleIndex(w http.ResponseWriter, r *http.Request) {
	...
	slug := htmlEscape(h.cfg.AppSlug)
	if user != nil {
		fmt.Fprintf(w, indexLoggedInHTML, htmlEscape(user.Login), slug, slug)
		return
	}
	fmt.Fprintf(w, indexAnonymousHTML, slug, slug)
}

const indexAnonymousHTML = `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>ClawFlow Cloud</title>
<style>
  body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
         max-width: 520px; margin: 6rem auto; padding: 0 1rem; color: #1f2328; }
  h1 { font-size: 1.5rem; margin-bottom: 0.5rem; }
  p { color: #57606a; line-height: 1.5; }
  a.btn { display: inline-block; background: #1f883d; color: #fff; padding: 0.55rem 1rem;
          border-radius: 6px; text-decoration: none; font-weight: 500; margin-top: 1rem; }
  a.btn:hover { background: #1a7f37; }
  code { background: #f6f8fa; padding: 0.1rem 0.3rem; border-radius: 4px; font-size: 0.9em; }
  .muted { color: #8c959f; font-size: 0.85rem; margin-top: 2.5rem; }
</style></head>
<body>
  <h1>ClawFlow Cloud</h1>
  <p>Label-driven automation for GitHub and GitLab. Sign in to register a
     worker machine or manage your repos.</p>
  <a class="btn" href="/api/v1/github/app/login">Sign in with GitHub</a>
  <p class="muted">Or from your terminal:<br>
     <code>clawflow cloud login --url https://clawflow.daboluo.cc</code></p>
  <p class="muted">App: <a href="https://github.com/apps/%s">github.com/apps/%s</a></p>
</body></html>`
...
<style>
  ...
  p { color: #57606a; line-height: 1.5; }
  code { background: #f6f8fa; padding: 0.1rem 0.3rem; border-radius: 4px; font-size: 0.9em; }
  .panel { background: #f6f8fa; border: 1px solid #d0d7de; border-radius: 8px;
           padding: 1rem 1.2rem; margin: 1.5rem 0; }
  .muted { color: #8c959f; font-size: 0.85rem; margin-top: 2.5rem; }
</style></head>
<body>
  <h1>Hi, %s 👋</h1>
  <p>You are signed in. The Web UI is not deployed on this cloud yet
     (coming in the next release).</p>
  <div class="panel">
    <p style="margin:0">From your terminal, register this machine:</p>
    <p style="margin:0.5rem 0 0"><code>clawflow worker register</code></p>
    <p style="margin:0.5rem 0 0"><code>clawflow worker start</code></p>
  </div>
  <p class="muted">App: <a href="https://github.com/apps/%s">github.com/apps/%s</a></p>
  <p class="muted">Sign out:
     <code>curl -X POST -b clawflow_session=&lt;cookie&gt; /api/v1/auth/logout</code></p>
</body></html>`
